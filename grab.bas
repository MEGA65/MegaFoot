   ...
   50 x=0:y=0:s=0:sv=0
   51 if peek($5fffe)<>71 or peek($5ffff)<>73 then wpoke $5fffe,$4947:poke $42000,0:rem check for magic marker at end of bank 5, if not there, initialise b4 mem
   55  cursor 0,49:gosub 1100:print cl$;"block count=";peek($42000);", next-free=$";hex$(z);
   60 if s=0 then t1=peek($40800+x+y*80):c1=peek($ff80000+x+y*80):x0=x:y0=y
   65 if s=1 then t2=peek($40800+x+y*80):c2=peek($ff80000+x+y*80):x1=x:y1=y
   70 poke $40800+x+y*80,43:poke $ff80000+x+y*80,1
   80 getkey a$
  ...
  120 if a$="{down}" and y<49 then y=y+1
  121 if a$="c" then print "{clr}";:b=0:gosub 620
  122 if a$="-" and ff>0 then ff=ff-1:b=ff:gosub 620
  124 if a$="=" and ff<peek($42000) then ff=ff+1:b=ff:gosub 620
  ...
  170 poke $40800+x0+y0*80,t1:poke $ff80000+x0+y0*80,c1
  180 poke $40800+x+y*80,t2:poke $ff80000+x+y*80,c2
  190 i=peek($42000):rem block count
  ...
  270 for yy=y0 to y
  280   for xx=x0 to x
  290     poke z,peek($40800+xx+yy*80):z=z+1
  310   next xx
  320 next yy
  321 for yy=y0 to y
  322   for xx=x0 to x
  323     poke z,peek($ff80000+xx+yy*80):z=z+1
  324   next xx
  325 next yy
  330 poke $42000,i+1
  350 cursor 0,49:print cl$;"block count=";peek($42000);", next-free=$";hex$(z);
  360 cursor 0,0:end
  600 rem *** load block ***
  610 print "{home}";cl$;:input "block#";b
  620 mx=peek($42000):z=$42001:i=0
  ...
  767 zz=z+w*h:p=$40800+x+y*80:pp=$ff80000+x+y*80
  770 for yy=y to y1
  780 edma 0, w, z, p
  790 edma 0, w, zz, pp
 ...
 1020   if s=0 then poke $40800+x+y*80,t1:poke $ff80000+x+y*80,c1
 1030   if s=1 then poke $40800+x+y*80,t2:poke $ff80000+x+y*80,c2
 1040 bend
 1045 return
 1049 rem *** show coords ***
 1050 cursor 0,49:print "                            ";:cursor 0,49
 ...
 1110 z=$42001:mx=peek($42000):i=0
 1120 if i=mx then l=z-$42000:return
 1130 z=z+2
 1140 w=peek(z):z=z+1
 1150 h=peek(z):z=z+1
 1160 z=z+w*h*2
